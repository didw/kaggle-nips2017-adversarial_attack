  ...
  for filenames, images in load_images(FLAGS.input_dir, [1]+batch_shape[1:]):
    ...
    for i in xrange(len(checkpoint_path_list)):
      ...
      with sess.as_default():
        ...
        images_clone = np.repeat(images, FLAGS.batch_size, axis=0)
        loss2 = sess.run(loss_list[i], feed_dict={x_input_list[i]: images_clone, y_list[i]: y_labels})
        ...
        losses_orig.append(loss2[0])
        losses.append(loss)
        print("loss: %f => rn: %f" % (np.max(loss2), np.max(loss)))
    loss_mean = np.mean(np.array(losses), axis=0)
    #print(np.shape(loss_mean))
    cand = loss_mean.argmax()
    #print(cand)
    print("%d's loss: %f => rn: %f" % (cand, np.mean(losses_orig), loss_mean[cand]))
